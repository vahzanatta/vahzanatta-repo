...
Private Sub ProcessarPendencias()
    ...
    lastRowBase = wsBase.UsedRange.Row + wsBase.UsedRange.Rows.Count - 1
    ...
    arrSAP  = wsSAP.Range("A2:K" & lastRowSAP).Value
    arrBase = wsBase.Range("A2:E" & lastRowBase).Value
    ...
    For i = 1 To totalRows
        contaSAP = Trim(CStr(arrSAP(i, 3)))
        textoSAP = Trim(CStr(arrSAP(i, 11)))
        dataLanc = arrSAP(i, 5)
        ...
        For j = 1 To lastRowBase - 1
            contaBase = Trim(CStr(arrBase(j, 1)))
            comecaEm  = Trim(CStr(arrBase(j, 2)))
            contem    = Trim(CStr(arrBase(j, 3)))
            regexStr  = Trim(CStr(arrBase(j, 4)))
            areaBase  = Trim(CStr(arrBase(j, 5)))
